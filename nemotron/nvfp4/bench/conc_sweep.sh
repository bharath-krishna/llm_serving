#!/usr/bin/env bash
# Concurrency sweep for the NVFP4 variant — run INSIDE the bench pod (see bench-pod.yaml).
#
...
# engine's unified block size so the shared prefix is exactly one cacheable
# page; that size is KV-dtype-derived, so it is NOT the same on BF16.
BASE_URL="http://vllm-nemotron-nvfp4.vllm.svc.cluster.local:8000"
